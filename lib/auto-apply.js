...
async function askUser(questions, jobContext) {
  ...
  // Automatically open the side panel so the user sees the question immediately
  try {
    const [currentTab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (currentTab?.windowId && chrome.sidePanel?.open) {
...
function flagSelectorsBroken(detail) {
  // Only stop the entire run if the search page itself has no job cards
  if (detail !== "no-job-cards") {
    return;
  }
  ...
  status("⚠️ Could not find any job cards on the search page. Check your search URL.");
}
...
async function processLinkedInEasyApply(job, tab, settings) {
  ...
  if (!opened?.ok) {
    if (opened?.reason === "already-applied") {
      status(`ℹ️ Already applied to ${job.company} — skipping.`);
      await finalizeApplied(job, tab, "already-applied", true);
      return;
    }
    const checkState = await sendToTab(tab.id, { type: "linkedinModalState" });
    if (!checkState?.inModal) {
      ...
      status(`⚠️ Could not open Easy Apply for ${job.company} (${opened?.reason || "button not clickable"}) — skipping to next.`);
      await pushLog({
        company: job.company,
        role: job.title,
        url: job.url,
        status: "Skipped",
        note: `Could not open Easy Apply: ${opened?.reason || "button not clickable"}`
      });
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    if (!state?.inModal) {
      ...
      if (state?.confirmed) {
        ...
        await pushLog({
          ...
          url: job.url,
          status: "Skipped",
          note: step === 0 ? "Apply flow closed immediately" : "Apply flow closed before reaching Submit"
        });
        status(`ℹ️ ${job.company} — ${job.title}: Apply flow closed early, skipping to next.`);
    ...
    if (lowRequired.length > 0) {
      ...
      const { answers = {}, remember = {}, skip = false } = await askUser(questions, {
        company: job.company,
        role: job.title,
        step: step + 1
      });
...
export async function handleAutoApplyMessage(msg) {
  switch (msg?.type) {
    ...
    case "user-answers":
      if (run.askWaiter) {
        run.askWaiter({ answers: msg.answers || {}, remember: msg.remember || {}, skip: !!msg.skip });
      }
      return { handled: true, response: { ok: true } };
