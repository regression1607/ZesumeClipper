...
async function startAutoApply({ criteria, jobBoardUrl, maxJobs }) {
  ...
  const board = /indeed\.com/i.test(jobBoardUrl || "") ? "indeed" : "linkedin";
  logRemote("run-start", {
    board,
    jobUrl: jobBoardUrl,
...
async function runLoop(settings) {
  ...
  status(
    `Found ${listings.jobs.length} Easy Apply listings (skipped ${meta.rejectedNonEasy || 0} external). Ranking against your criteria...`
  );
  const ranking = await rankJobs(listings.jobs, run.criteria, settings);
  ...
  status(`Matched ${run.matchedJobs.length} of ${listings.jobs.length} jobs.`);
...
async function rankJobs(listings, criteria, settings) {
  const { system, user } = buildRankingPrompt(listings, criteria);
  const result = await callLLM(system, user, settings.provider, settings.apiKey);
  if (!Array.isArray(result)) throw new Error("Ranking response was not a JSON array");
  return result;
}

async function processJob(job, settings) {
  status(`Opening: ${job.company} — ${job.title}`);
  const isLinkedIn = /(^|\.)linkedin\.com$/i.test(new URL(job.url).hostname);
  logRemote("job-open", {
    board: isLinkedIn ? "linkedin" : "other",
    company: job.company,
    role: job.title,
    jobUrl: job.url,
  });
...
export async function handleAutoApplyMessage(msg) {
  switch (msg?.type) {
    ...
    case "approve-held-jobs":
      approveHeldJobs().then(() => { if (run.waiter) run.waiter(); });
      return { handled: true, response: { ok: true } };
    case "skip-held-jobs":
      skipHeldJobs().then(() => { if (run.waiter) run.waiter(); });
      return { handled: true, response: { ok: true } };
    case "user-answers":
      if (run.askWaiter) {
