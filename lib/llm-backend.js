...
function tryParseJson(text) {
  ...
  try {
    ...
    // Attempt recovery for truncated JSON arrays: e.g. [ {...}, {...
    if (cleaned.includes("[")) {
      const startIdx = cleaned.indexOf("[");
      const lastCloseBrace = cleaned.lastIndexOf("}");
      if (lastCloseBrace > startIdx) {
        try {
          return JSON.parse(cleaned.slice(startIdx, lastCloseBrace + 1) + "]");
        } catch (_) {}
      }
    }
    // Attempt recovery for truncated JSON objects: e.g. { "foo": "bar", ...
    if (cleaned.includes("{")) {
      const startIdx = cleaned.indexOf("{");
      const lastQuote = cleaned.lastIndexOf('"');
      if (lastQuote > startIdx) {
        try {
          return JSON.parse(cleaned.slice(startIdx, lastQuote + 1) + "}");
        } catch (_) {}
      }
    }
